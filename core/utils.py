...
class Submission(object):
    ...
    def __init__(self, df_pool, df_samples, output):
        ...
        df_pool.to_excel(
            self.writer,
            sheet_name='Submission Info',
            startrow=self.pool_start,
            startcol=0,
            header=False,
            index=False,
        )
    ...
    def write_address_box(self, info_dict):
        ...
        HEADER = [
            "Deliver/ship samples on dry ice or ice pack to:",
            "%s" % info_dict['name'],
            "%s" % info_dict['org'],
            "%s" % info_dict['org_gsc'],
        ...
        lower_border = self.workbook.add_format({
            'font_color': 'black',
            'pattern': True,
            'bg_color': 'white',
            'bottom': 6
        })
        right_border = self.workbook.add_format({
            'font_color': 'black',
            'pattern': True,
            'bg_color': 'white',
            'right': 6
        })
        lower_right_border = self.workbook.add_format({
            'font_color': 'black',
            'pattern': True,
            'bg_color': 'white',
            'bottom': 6,
        ...
        light_green = self.workbook.add_format({
            'pattern': True,
            'bold': True,
            'align': 'right',
            'bg_color': '#E5F6D9',
        ...
        peach = self.workbook.add_format({
            'pattern': True,
            'bold': True,
            'align': 'right',
            'bg_color': '#F7C876',
        ...
        self.worksheet.write(
            input_cell.format(column="A", row=row),
            "PLEASE PROVIDE COMPLETE INFORMATION FOR YOUR SAMPLES IN THE FIELDS BELOW.  ENTER \"N/A\" IN FIELDS THAT DO NOT APPLY TO YOUR SAMPLES.",
            red)
        ...
        row += 2
        self.worksheet.write(
            input_cell.format(column="A", row=row), "Is this Chromium library?",
        ...
        row += 1
        self.worksheet.write(
            input_cell.format(column="A", row=row), "PLEASE NOTE:",
            self.workbook.add_format({
                'pattern': True,
                'bold': True,
                'align': 'right',
        ...
        self.worksheet.write(
            input_cell.format(column="B", row=row),
            "If yes, please provide specific chromium sample index name in Column AE. Here are some helpful links:",
            self.workbook.add_format({
                'bold': True,
                'font_color': 'red',
                'align': 'left'
        ...
        self.worksheet.write(
            input_cell.format(column="B", row=row),
            "http://support.10xgenomics.com/single-cell/sequencing/doc/specifications-sample-index-sets-for-single-cell-3"
        )
        ...
        self.worksheet.write(
            input_cell.format(column="B", row=row),
            "http://support.10xgenomics.com/genome-exome/sequencing/doc/specifications-sample-index-sets-for-genome-and-exome"
        )

        row += 2
        self.worksheet.write(input_cell.format(column="A", row=row), "For Custom Library Info only:", dark_green)
        row += 1
        self.worksheet.write(input_cell.format(column="A", row=row), "Primer 1 Name:", light_green)
        row += 1
        self.worksheet.write(input_cell.format(column="A", row=row), "Primer 1 Sequence (with 5' and 3'):", light_green)
        row += 1
        self.worksheet.write(input_cell.format(column="A", row=row), "Primer 2 Name:", light_green)
        row += 1
        self.worksheet.write(input_cell.format(column="A", row=row), "Primer 2 Sequence (with 5' and 3'):", light_green)
        row += 1
        self.worksheet.write(input_cell.format(column="A", row=row), "Adaptor 1 Name:", light_green)
        row += 1
        self.worksheet.write(input_cell.format(column="A", row=row), "Adaptor 1 Sequence (with 5' and 3'):",
                             light_green)
        row += 1
        self.worksheet.write(input_cell.format(column="A", row=row), "Adaptor 2 Name:", light_green)
        row += 1
        self.worksheet.write(
            input_cell.format(column="A", row=row),
            "Adaptor 2 Sequence (with 5' and 3'):",
            light_green,
        )
        row += 2
        ...
        self.worksheet.write(
            input_cell.format(column="A", row=row),
            "At completion of project (choose one):",
            right_align,
        )
        self.worksheet.write(input_cell.format(column="B", row=row), at_completion)
        self.worksheet.write(
            input_cell.format(column="C", row=row),
            "=IF(EXACT(B45, \"Destroy Unused Sample\"), \"GSC will destroy any remaining sample at completion of project\", IF(EXACT(B45,\"Return Unused Sample\"), \"GSC will return any residual sample at Submitter's expense\",\"\"))",
            bold)

        row += 3
        self.worksheet.write(
            input_cell.format(column="A", row=row),
            "Sample Requirements (Volume & Amounts):",
            right_align,
        )
        self.worksheet.write(
            input_cell.format(column="B", row=row),
            "http://www.bcgsc.ca/services/sequencing-libraries-faq",
        )
        ...
        self.worksheet.write(
            input_cell.format(column="B", row=row),
            "If indices are supplied, the reads will automatically be split by index.",
            bold,
        )
        ...
        self.worksheet.write(
            input_cell.format(column="B", row=row),
            "If the reads require splitting by index, but indices are not supplied or are incorrect, there will be a 1-2 week delay in the data processing.",
            bold)
        ...
        self.worksheet.write_rich_string(
            input_cell.format(column="A", row=row),
            bold,
            'Enter POOL details below',
            yellow_fill,
        )

    def write_pool_header(self, headers):
        #Formats
        header_format_light = self.workbook.add_format({
            'align': 'center',
            'pattern': True,
            'fg_color': '#DCE6F1',
            'bold': 1,
        ...
        header_format_dark = self.workbook.add_format({
            'align': 'center',
            'pattern': True,
            'fg_color': '#B9CDE3',
            'bold': 1,
        ...
        subheader_format_light = self.workbook.add_format({
            'align': 'center',
            'pattern': True,
            'fg_color': '#DCE6F1',
            'bold': 1,
        ...
        subheader_format_dark = self.workbook.add_format({
            'align': 'center',
            'pattern': True,
            'fg_color': '#B9CDE3',
            'bold': 1,
        ...
        comment_format = self.workbook.add_format({
            'align': 'center',
            'pattern': True,
            'fg_color': '#D89595',
            'bold': 1,
        ...
        for k, v in headers.items():
            ...
            if k == "Additional Comments":
                input_cell = span.format(
                    column=self.columns[column_count],
                    row=row_num,
                    column_end=self.columns[column_count + len(v) - 1],
                    row_end=(row_num + 1),
                )
                comment_cell = span.format(
                    column=self.columns[column_count],
                    row=row_num + 2,
                    column_end=self.columns[column_count + len(v) - 1],
                    row_end=(row_num + 2),
                ...
                for k2, v2 in v.items():
                    input_cell = span.format(
                        column=self.columns[subheader_count],
                        row=row_num + 1,
                        column_end=self.columns[subheader_count],
                        row_end=row_num + 1,
                    )
                    if header_count % 2 == 0:
    ...
    def write_sample_header(self, headers):
        ...
        subheader_format_light_blue = self.workbook.add_format({
            'pattern': True,
            'fg_color': '#EBF1DF',
            'bold': 1,
            'border': 1,
        ...
        subheader_format_dark_blue = self.workbook.add_format({
            'pattern': True,
            'fg_color': '#D8E3BD',
            'bold': 1,
            'border': 1,
        ...
        subheader_format_light_red = self.workbook.add_format({
            'pattern': True,
            'fg_color': '#EBF1DF',
            'bold': 1,
            'border': 1,
        ...
        subheader_format_dark_red = self.workbook.add_format({
            'pattern': True,
            'fg_color': '#D8E3BD',
            'bold': 1,
            'border': 1,
